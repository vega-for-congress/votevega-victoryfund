...
(function () {
  ...
  // ===========================================
  // HELPER: Get total raised (Stripe + non-Stripe)
  // ===========================================
  function getTotalRaised(stripeAmount) {
    return (stripeAmount || 0) + CONFIG.nonStripeAmount;
  }
  ...
  // THERMOMETER
  // ===========================================
  var currentStripeAmount = CONFIG.mode === 'live' ? 0 : CONFIG.startingAmount;
  var currentDonors = CONFIG.mode === 'live' ? 0 : CONFIG.startingDonors;

  function updateThermometer() {
    var totalRaised = getTotalRaised(currentStripeAmount);
    var pct = Math.min((totalRaised / CONFIG.goal) * 100, 100);

    setText('thermo-raised', '$' + formatNumber(totalRaised));
    ...
    // Update milestone labels to match CONFIG.goal
  ...
  // ===========================================
  // DONATION FEED (simulated only — non-Stripe not shown in feed)
  // ===========================================
  ...
  // Seed initial donations (demo mode only)
  function seedFeeds() {
  ...
  function simulateNewDonation() {
    ...
    currentStripeAmount += donation.amount;
    currentDonors += 1;
    updateThermometer();
    ...
    if (miniFeedList) {
      ...
      while (miniFeedList.children.length > 4) miniFeedList.removeChild(miniFeedList.lastChild);
    }
    ...
    if (donationFeed) {
      ...
      while (donationFeed.children.length > 30) donationFeed.removeChild(donationFeed.lastChild);
    }
  }

  // ===========================================
  // STICKY BAR & COPY LINK (unchanged)
  // ===========================================
  ...
  function showLoadingState() {
    ...
    if (miniFeedList) {
      ...
      for (var i = 0; i < 4; i++) {
        ...
        skeleton.innerHTML = '<span class="mb-skeleton__bar" style="width:60%"></span><span class="mb-skeleton__bar" style="width:20%"></span>';
    ...
    if (donationFeed) {
      ...
      for (var j = 0; j < 5; j++) {
        ...
        sk.innerHTML = '<div class="mb-feed__avatar mb-skeleton__circle"></div><div class="mb-feed__content"><div class="mb-skeleton__bar" style="width:45%;margin-bottom:8px"></div><div class="mb-skeleton__bar" style="width:80%"></div></div>';
  ...
  function fetchLiveData() {
    return fetch(buildApiUrl())
      ...
      .then(function (data) {
        if (data.countdownEnd) CONFIG.countdownEnd = new Date(data.countdownEnd);
        if (data.goal) CONFIG.goal = data.goal;

        currentStripeAmount = data.raised || 0;
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    updateThermometer();   // initial render
